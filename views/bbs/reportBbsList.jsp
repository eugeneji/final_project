...
<link href="css/bbs/reportbbs.css" rel="stylesheet"/>
<style>
.comment_writer{
...
<script>
function content(bbs_idx) {
	var params='bbs_idx='+bbs_idx;
	sendRequest('reportBbsContent.do',params,showResult,'GET');
}
function showResult() {
	if(XHR.readyState==4){
		if(XHR.status==200){
			...
			document.getElementById('typeTd').innerHTML=data.dto.bbs_type==1?"강아지":"고양이";
			...
			document.getElementById('editbutton').onclick = function(){location.href='reportBbsEdit.do?bbs_idx='+data.dto.bbs_idx};
...
function show(cp){
	location.href='reportBbsList.do?cp='+cp;
...
<%@ include file="/WEB-INF/views/header.jsp" %>
	...
	<h1>
  유기동물
  <small class="text-muted">제보하기</small>
</h1>
	<div id="reportList">
	<table class="table" aling="center" style="margin-left: auto; margin-right: auto;">
		...
		<tfoot>
			<tr>
				...
				<td><a href="reportBbsWrite.do">글쓰기</a></td>
			</tr>
		</tfoot>
		<tbody>
			...
			<c:forEach var="dto" items="${list}">
				<tr>
					<td >${dto.bbs_idx}</td>
					<c:url var="contenUrl" value="reportBbsContent.do">
						<c:param name="bbs_idx">${dto.bbs_idx}</c:param>
					</c:url>
					<td><a style="overflow:hidden" href="javascript:content(${dto.bbs_idx})">${dto.bbs_subject}</a></td>
	...
	<div id="contentLayer"
		...
		<div id="comment">
			...
			<input id="comment_bbs_idx" type="hidden" name="bbs_idx">
				<div>
					<input type="text" id="comment_content" name="comment_content" style="width: 710px;">
